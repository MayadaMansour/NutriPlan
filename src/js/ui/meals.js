const API = "https://nutriplan-api.vercel.app/api";
const sections = [
  document.getElementById("search-filters-section"),
  document.getElementById("meal-categories-section"),
  document.getElementById("all-recipes-section"),
];

const grid = document.getElementById("recipes-grid");
const count = document.getElementById("recipes-count");
const searchInput = document.getElementById("search-input");

let openDetailsFn;

export function showMealsPage() {
  sections.forEach((section) => {
    if (section) section.style.display = "block";
  });
}

export function hideMealsPage() {
  sections.forEach((section) => {
    if (section) section.style.display = "none";
  });
}

export function initMeals(openDetails) {
  openDetailsFn = openDetails;

  fetchMeals();

  grid.onclick = (event) => {
    const card = event.target.closest(".recipe-card");
    if (!card) return;

    hideMealsPage();
    openDetailsFn(card.dataset.mealId);
  };

  searchInput.oninput = (e) => {
    const value = e.target.value.trim();

    if (!value) {
      fetchMeals();
    } else {
      searchMeals(value);
    }
  };
}

async function searchMeals(query) {
  const response = await fetch(
    `${API}/meals/search?q=${query}&page=1&limit=25`
  );

  const data = await response.json();
  renderMeals(data.results || []);
}

export async function fetchMeals() {
  const response = await fetch(`${API}/meals/search?q=chicken&page=1&limit=25`);

  const data = await response.json();
  renderMeals(data.results || []);
}

export async function fetchMealsByFilter(params) {
  const response = await fetch(`${API}/meals/filter?${params}&page=1&limit=25`);

  const data = await response.json();
  renderMeals(data.results || []);
}

export function renderMeals(meals) {
  grid.innerHTML = "";
  count.textContent = `Showing ${meals.length} recipes`;

  if (meals.length === 0) {
    grid.innerHTML = `
      <p class="col-span-full text-center text-gray-400">
        No meals found
      </p>
    `;
    return;
  }

  meals.forEach((meal) => {
    grid.innerHTML += `
      <div class="recipe-card bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-lg cursor-pointer"
           data-meal-id="${meal.id}">

        <div class="relative h-48">
          <img src="${meal.thumbnail}" alt="${meal.name}"
               class="w-full h-full object-cover" />

          <div class="absolute bottom-3 left-3 flex gap-2">
            <span class="px-2 py-1 bg-white text-xs rounded-full">
              ${meal.category}
            </span>
            <span class="px-2 py-1 bg-emerald-500 text-xs rounded-full text-white">
              ${meal.area}
            </span>
          </div>
        </div>

        <div class="p-4">
          <h3 class="font-bold">${meal.name}</h3>
          <p class="text-xs text-gray-600">
            Delicious recipe to try!
          </p>
        </div>
      </div>
    `;
  });
}
